   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_778)
            {
               if(var_717 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + var_717.message,var_717.errorID,var_717);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,var_717);
               }
            }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!_disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_2301 == param3)
                  {
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_2301 == param3)
               {
                  return;
               }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1115) : uint(const_507);
      }
